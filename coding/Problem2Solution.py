...
MODEL.addConstrs(
    sum(x[i, j, k1, j_, k2] + x[i, j_, k1, j, k2] for i in I for j_ in J for k1 in K for k2 in K) <= 1 for j in J)
MODEL.addConstrs(
    sum(x[i, j1, k_, j2, k] + x[i, j1, k, j2, k_] for i in I for j1 in J for k_ in K for j2 in J) <= 8 for k in K)
MODEL.addConstrs(t[i] == sum((cost_matrix[alpha[i]].at[beta[i], j1] + cost_matrix[alpha[i]].at[j1, k1] +
                              cost_matrix[alpha[i]].at[k1, j2] + cost_matrix[alpha[i]].at[j2, k2]) * x[
                                 i, j1, k1, j2, k2] for j1 in J for k1 in K for j2 in J for k2 in K) + 2 * w[alpha[i]]
                 for i in I)
