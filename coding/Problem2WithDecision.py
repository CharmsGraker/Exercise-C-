...
# 只能去同一个最多一次
MODEL.addConstrs(
    sum(x_1[i, j, k1] for i in I for k1 in K) + sum(x_2[i, k1, j, k2] for i in I for k1 in K for k2 in K) <= 1 for j in
    J)
# min(min()+min())
...
MODEL.addConstrs(t[i] == sum(
    ...
    K) +
                 sum(x_2[i, k1, j2, k2] * (cost_matrix[alpha[i]].at[k1, j2] + cost_matrix[alpha[i]].at[j2, k2]) for k1
                     in K for j2 in J for k2 in K) + 2 * w[alpha[i]] for i in I)
